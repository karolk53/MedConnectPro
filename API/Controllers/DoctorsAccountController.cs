...
namespace API.Controllers
{
    ...
    public class DoctorsAccountController : BaseApiController
    {
        ...
        public async Task<ActionResult<DoctorDto>> Register(DoctorRegisterDto registerDto)
        {
            ...
            if(!ValidPWZ(registerDto.PWZ)) return BadRequest("Invalid PWZ");
        ...
        public bool ValidPWZ(string pwz)
        {
            var suma = 0;
            var temp = 0;

            foreach(char c in pwz){
                suma += (int) char.GetNumericValue(c) * temp;
                temp++;
            }
            
            if (suma % 11 == (int) char.GetNumericValue(pwz[0])) return true;
